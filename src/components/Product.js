                        ...
                        {value =>(<div className='img-container p-5 ' onClick={() => { value.handleDetail(id) }}>
                                ...
                                <button className='cart-btn' disabled={inCart ? true : false}
                                    onClick={() => { value.addToCart(id);
                                                     value.openModal(id)}} >
                                    {inCart ? (<p className='text-capitalize mb-0' disabled>incart</p>) : (<FontAwesomeIcon icon={faCartPlus} ></FontAwesomeIcon>)}
                                </button></div>)}
                    </ProductConsumer>
                    {/*footer*/}
...
const ProductWrapper = styled.div`
   .card{
       ...
       &:hover{
           .card{
               border: 0.04rem solid rgba(0,0,0,0.4) !important;
